...
style_plot = [[230/255,159/255,0],  [86/255,180/255,233/255], [0,158/255,115/255],[213/255,94/255,0],[0,114/255,178/255]]
...
sql = """
SELECT dataset,plan,threads,sequential_time, avg_time,stdev,1 as o FROM ( 
select dataset,plan,threads as threads, (select avg(total_time) from result as result_sequential where result_sequential.mode=  result.mode and result_sequential.plan='CPU_SEQUENTIAL_ORIGINAL' and result_sequential.dataset = result.dataset) as sequential_time, avg(total_time)  as avg_time, STDEV(total_time) as stdev
...
AND plan = 'CPU_SEQUENTIAL' and plan <> 'CPU_SEQUENTIAL_ORIGINAL' AND dataset like "{1}"
 ...
 UNION 
SELECT dataset,plan,threads,sequential_time, avg_time,stdev,2 as o FROM ( 
select dataset,plan,threads as threads, (select avg(total_time) from result as result_sequential where result_sequential.mode=  result.mode and result_sequential.plan='CPU_SEQUENTIAL_ORIGINAL' and result_sequential.dataset = result.dataset) as sequential_time, avg(total_time)  as avg_time, STDEV(total_time) as stdev
...
select dataset,plan,threads as threads, (select avg(total_time) from result as result_sequential where result_sequential.mode=  result.mode and result_sequential.plan='CPU_SEQUENTIAL_ORIGINAL' and result_sequential.dataset = result.dataset ) as sequential_time, avg(total_time)  as avg_time, STDEV(total_time) as stdev,3 as o

 from result where mode='{0}' and plan <> 'CPU_ORIGINAL'
AND plan <> 'CPU_SEQUENTIAL' and plan <> 'CPU_SEQUENTIAL_ORIGINAL' AND dataset like "{1}"
 group by dataset,plan,threads

 order by o,dataset,threads,plan"""
...
for dataset in datasets:
    for mode in ["memory","file"]:
        with DatasetHelper() as db:
            ...
            fig, ax = plt.subplots()
            thread_8 = [values[0]]
            thread_8_err = [y_error[0]]
            thread_max = [values[1]]
            thread_max_err = [y_error[1]]
            gpu_1 = [values[2], values[3]]
            gpu_1_err = [y_error[2], y_error[3]]
            gpu_2 = [values[4], values[5]]
            gpu_2_err = [y_error[4], y_error[5]]
            
            gpu_3 = [values[6], values[7]]
            gpu_3_err = [y_error[6], y_error[7]]
            
            gpu_4 = [values[8], values[9]]
            gpu_4_err = [y_error[8], y_error[9]]
            
            rect = ax.bar([0], thread_8,align='center',edgecolor="black",yerr = thread_8_err ,color=style_plot[2],error_kw=dict(ecolor='black',
                lw=2, capsize=5, capthick=2),hatch=hatches[2],label="40 Threads")
            autolabel(ax,rect)

            rect = ax.bar([1], thread_max,align='center',edgecolor="black",yerr = thread_max_err ,color=style_plot[3],error_kw=dict(ecolor='black', lw=2, capsize=5, capthick=2),hatch=hatches[3],label="{0} Threads".format(data[1][2]))
            autolabel(ax,rect)
            ...
            rect = ax.bar([6,7], gpu_3,align='center',edgecolor="black",yerr = gpu_3_err ,color=style_plot[2],error_kw=dict(ecolor='black', lw=2, capsize=5, capthick=2),hatch=hatches[2],label="3 GPUs")
            autolabel(ax,rect)


            rect = ax.bar([8,9], gpu_4,align='center',edgecolor="black",yerr = gpu_4_err ,color=style_plot[3],error_kw=dict(ecolor='black', lw=2, capsize=5, capthick=2),hatch=hatches[3],label="4 GPUs")
            autolabel(ax,rect)
