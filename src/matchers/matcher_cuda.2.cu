...
__global__
void  FindMatchBatchKernel(char* buffer, int bufferSize, int* matches_length, int* matches_offset, int bufferSizeAdjusted,int currentMatchCount, bool isLast) {
...
int error = 0;
        for(int k = 0; k < WINDOW_SIZE ;k++){
            if(beginSearch + k < bufferSize && buffer[beginSearch + k] != bufferCache[beginInBufferCache+ k])
                error ++;
        }
    if(error > 0)
            printf("%i %i\n",error,idX);
