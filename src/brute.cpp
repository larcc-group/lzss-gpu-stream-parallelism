...
encoded_string_t FindMatch(unsigned char * slidingWindow, unsigned char * uncodedLookahead, const unsigned int windowHead,
    unsigned int uncodedHead, int currentCharCount)
{
    encoded_string_t matchData;
    unsigned int i;
    unsigned int j;
    ...
    i = windowHead;  /* start at the beginning of the sliding window */
    j = 0;
    // if(currentCharCount < WINDOW_SIZE ){
    //     i = 0;
    // }
    // if(i > 1024){ exit(-1);}
// if(currentCharCount < WINDOW_SIZE - MAX_CODED)
//         printf("a i: %i  WINDOW_SIZE - MAX_CODED: %i currentCharCount: %i\n",i, WINDOW_SIZE - MAX_CODED,currentCharCount );
    
    //Escape unnecessary searches on slidingWindow empty
    // if(currentCharCount < WINDOW_SIZE && i < WINDOW_SIZE ){
    //     // std::cout << "Escaping from "<< i << " to " << WINDOW_SIZE - MAX_CODED << "\n";
    //     // printf("Escaping from %i to %i\n",i, WINDOW_SIZE - MAX_CODED );
    //     printf("Escaping from: \n");
    //     for(int k = 0; k < MAX_CODED + 10; k ++){
    //         printf("%c",slidingWindow[Wrap((k+ i), WINDOW_SIZE)]);
    //     }
    //     printf("\nTo: \n");
...
*                event of a failure.
****************************************************************************/
int ReplaceChar(unsigned char * slidingWindow, const unsigned int charIndex, const unsigned char replacement)
{
    slidingWindow[charIndex] = replacement;
    return 0;
}
